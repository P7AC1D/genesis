...
namespace Genesis {
    ...
    float TerrainGenerator::SampleHeight(float x, float z) const {
        ...
        float height = m_Noise.FBM(noiseX, noiseZ, 
            m_Settings.octaves, 
        ...
        if (m_Settings.flatShading) {
            ...
            for (int z = 0; z < m_Settings.depth; z++) {
                for (int x = 0; x < m_Settings.width; x++) {
                    ...
                    glm::vec3 color1 = m_Settings.useHeightColors ? 
                        GetHeightColor(normH1, m_Settings) : glm::vec3(0.34f, 0.55f, 0.25f);
                    ...
                    glm::vec3 color2 = m_Settings.useHeightColors ? 
                        GetHeightColor(normH2, m_Settings) : glm::vec3(0.34f, 0.55f, 0.25f);
            ...
            for (int z = 0; z < depth; z++) {
                for (int x = 0; x < width; x++) {
                    ...
                    glm::vec3 color = m_Settings.useHeightColors ? 
                        GetHeightColor(normH, m_Settings) : glm::vec3(0.34f, 0.55f, 0.25f);
